...
test('invitaciones y comentarios validan sesión y propiedad', () => { const source = readFileSync('api/_lib/collaboration.js', 'utf8'); assert.match(source, /requireUser\(req\)/); assert.match(source, /Solo el propietario puede invitar/); assert.match(source, /No tienes acceso a la tarea/); });
import test from 'node:test';
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
test('las APIs privadas exigen sesión y filtran por propietario', () => { for (const file of ['api/projects.js', 'api/tasks.js', 'api/dashboard.js']) { const source = readFileSync(file, 'utf8'); assert.match(source, /requireUser\(req\)/); assert.match(source, /owner_id/); } });
