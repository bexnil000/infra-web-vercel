...
document.querySelector('#projectForm').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const id = form.dataset.editId; try { await api(id ? `/api/projects?id=${encodeURIComponent(id)}` : '/api/projects', { method: id ? 'PUT' : 'POST', body: JSON.stringify(formData(form)) }); form.reset(); delete form.dataset.editId; document.querySelector('#projectDialog').close(); await loadDashboard(); showAlert(id ? 'Proyecto actualizado.' : 'Proyecto creado.'); } catch (error) { showAlert(error.message, 'error', form.querySelector('.form-alert')); } });
document.querySelector('#taskForm').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; try { await api('/api/tasks', { method: 'POST', body: JSON.stringify(formData(form)) }); form.reset(); document.querySelector('#taskDialog').close(); await loadDashboard(); showAlert('Tarea creada.'); } catch (error) { showAlert(error.message, 'error', form.querySelector('.form-alert')); } });
document.querySelector('#profileForm').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = formData(form); if (data.new_password !== data.password_confirmation) return showAlert('Las contraseñas no coinciden.', 'error', form.querySelector('.form-alert')); try { await api('/api/profile', { method: 'PUT', body: JSON.stringify(data) }); await loadDashboard(); showAlert('Perfil actualizado.', 'success', form.querySelector('.form-alert')); } catch (error) { showAlert(error.message, 'error', form.querySelector('.form-alert')); } });
document.querySelector('#inviteForm').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; try { await api('/api/collaboration', { method: 'POST', body: JSON.stringify({ action: 'invite', ...formData(form) }) }); form.elements.email.value = ''; await loadDashboard(); showAlert('Colaborador agregado.'); } catch (error) { showAlert(error.message, 'error'); } });
